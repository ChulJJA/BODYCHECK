...
void PLAYER_UI::Initialize()
{
	...
	info_item->AddComponent(new Sprite(info_item, "../Sprite/Item.png", { this_trans.x + 200, this_trans.y - 200 }), "missile", false);
...
void PLAYER_UI::Reset()
{
	...
	info_hp_bar->GetTransform().SetTranslation({ this_trans.x + 400, this_trans.y + 100 });
	info_hp_bar->GetTransform().SetScale({ 4.f, 8.f });
	...
	info_item->GetTransform().SetTranslation({ this_trans.x + 400, this_trans.y - 200 });
